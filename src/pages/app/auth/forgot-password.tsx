...
import { useForgot_password_initiateMutation } from "@app/store/services/authenticationslice";
import Link from "next/link";
import SuccessUI from "@app/components/app/authentication/success_ui";

const Page = () => {
  const router = useRouter();
  const [forgot_password_initiate, { isLoading, isSuccess }] =
  ...
  const handleSubmit = async (
    ...
    setErrorMsg(null);
    try {
      const res = await forgot_password_initiate({
        ...
        <div className=" relative h-full lg:flex items-center">
          ...
          {isSuccess ? (
            <SuccessUI
              ...
              subheading="Please check your mail box for the reset link"
              link="/"
            />
          ) : (
